...
@protocol ZC_FLAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(ZC_FLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface ZC_FLAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<ZC_FLAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation ZC_FLAnimatedImageView
...
- (void)setAnimatedImage:(ZC_FLAnimatedImage *)animatedImage
{
...
{
    // Presision is set to half of the `kZC_FLAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kZC_FLAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            ZC_FLWeakProxy *weakProxy = [ZC_FLWeakProxy weakProxyForObject:self];
