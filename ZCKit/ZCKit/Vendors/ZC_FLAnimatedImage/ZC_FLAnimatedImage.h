...
extern const NSTimeInterval kZC_FLAnimatedImageDelayTimeIntervalMinimum;

//
//  An `ZC_FLAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `ZC_FLAnimatedImageView`.
...
//
@interface ZC_FLAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `ZC_FLAnimatedImage` and get back its size
+ (CGSize)sizeForImage:(id)image;

// On success, the initializers return an `ZC_FLAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@interface ZC_FLAnimatedImage (Logging)
...
#define FLLog(logLevel, format, ...) [ZC_FLAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface ZC_FLWeakProxy : NSProxy
