...
@implementation UIImage (ZCSD_WebP)
...
+ (nullable UIImage *)zc_sd_imageWithWebPData:(nullable NSData *)data {
    ...
    if (!(flags & ANIMATION_FLAG)) {
        ...
        UIImage *staticImage = [self zc_sd_rawWepImageWithData:webpData];
    ...
    do {
        ...
        if (iter.blend_method == WEBP_MUX_BLEND) {
            image = [self zc_sd_blendWebpImageWithOriginImage:[images lastObject] iterator:iter];
        } else {
            image = [self zc_sd_rawWepImageWithData:iter.fragment];
        }
...
+ (nullable UIImage *)zc_sd_blendWebpImageWithOriginImage:(nullable UIImage *)originImage iterator:(WebPIterator)iter {
    if (!originImage) {
        return nil;
    }
    ...
    UIImage *image = [self zc_sd_rawWepImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
+ (nullable UIImage *)zc_sd_rawWepImageWithData:(WebPData)webpData {
