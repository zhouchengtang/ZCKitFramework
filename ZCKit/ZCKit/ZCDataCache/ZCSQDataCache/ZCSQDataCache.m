...
@implementation ZCSQDataCacheItem
...
@interface ZCSQDataCache()

@property (strong, nonatomic) ZCFMDatabaseQueue * dbQueue;

@end

@implementation ZCSQDataCache
...
- (id)initDBWithName:(NSString *)dbName {
    ...
    if (self) {
        ...
        _dbQueue = [ZCFMDatabaseQueue databaseQueueWithPath:dbPath];
    }
    return self;
}
...
- (void)createTableWithName:(NSString *)tableName {
    if ([ZCSQDataCache checkTableName:tableName] == NO) {
        return;
    }
    NSString * sql = [NSString stringWithFormat:CREATE_TABLE_SQL, tableName];
    __block BOOL result;
    [_dbQueue inDatabase:^(ZCFMDatabase *db) {
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    ZCSQDataCacheItem * item = [self getZCKeyValueItemById:objectId fromTable:tableName];
    if (item) {
...
- (ZCSQDataCacheItem *)getZCKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    [_dbQueue inDatabase:^(ZCFMDatabase *db) {
        ZCFMResultSet * rs = [db executeQuery:sql, objectId];
        if ([rs next]) {
    ...
    if (json) {
        ...
        ZCSQDataCacheItem * item = [[ZCSQDataCacheItem alloc] init];
...
- (void)setString:(NSString *)string withId:(NSString *)stringId intoTable:(NSString *)tableName {
    ...
    [self setObject:@[string] withId:stringId intoTable:tableName];
}
...
- (void)setNumber:(NSNumber *)number withId:(NSString *)numberId intoTable:(NSString *)tableName {
    ...
    [self setObject:@[number] withId:numberId intoTable:tableName];
}
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    [_dbQueue inDatabase:^(ZCFMDatabase *db) {
        ZCFMResultSet * rs = [db executeQuery:sql];
        while ([rs next]) {
            ZCSQDataCacheItem * item = [[ZCSQDataCacheItem alloc] init];
    ...
    NSError * error;
    for (ZCSQDataCacheItem * item in result) {
        error = nil;
