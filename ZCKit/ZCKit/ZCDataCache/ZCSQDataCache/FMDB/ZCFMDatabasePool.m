...
@interface ZCFMDatabasePool()

- (void)pushDatabaseBackInPool:(ZCFMDatabase*)db;
- (ZCFMDatabase*)db;

@end


@implementation ZCFMDatabasePool
...
+ (instancetype)databasePoolWithPath:(NSString*)aPath flags:(int)openFlags {
    return ZCFMDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags {
    ...
    if (self != nil) {
        ...
        _databaseInPool     = ZCFMDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = ZCFMDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    ZCFMDBRelease(_path);
    ZCFMDBRelease(_databaseInPool);
    ZCFMDBRelease(_databaseOutPool);

    if (_lockQueue) {
        ZCFMDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(ZCFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The ZCFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (ZCFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [ZCFMDatabase databaseWithPath:self->_path];
