...
#import "ZCFMDatabasePool.h"
    ...
    #define ZCFMDBAutorelease(__v) ([__v autorelease]);
    #define ZCFMDBReturnAutoreleased ZCFMDBAutorelease

    #define ZCFMDBRetain(__v) ([__v retain]);
    #define ZCFMDBReturnRetained ZCFMDBRetain

    #define ZCFMDBRelease(__v) ([__v release]);

    #define ZCFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define ZCFMDBAutorelease(__v)
    #define ZCFMDBReturnAutoreleased(__v) (__v)

    #define ZCFMDBRetain(__v)
    #define ZCFMDBReturnRetained(__v) (__v)

    #define ZCFMDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details.
    #if OS_OBJECT_USE_OBJC
        #define ZCFMDBDispatchQueueRelease(__v)
    #else
        #define ZCFMDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `ZCFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<ZCFMResultSet>` - Represents the results of executing a query on an `ZCFMDatabase`.
 - `<ZCFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also

 - `<ZCFMDatabasePool>` - A pool of `ZCFMDatabase` objects.
 - `<ZCFMStatement>` - A wrapper for `sqlite_stmt`.
 ...
 @warning Do not instantiate a single `ZCFMDatabase` object and use it across multiple threads. Instead, use `<ZCFMDatabaseQueue>`.

 */

@interface ZCFMDatabase : NSObject  {
...
/** Create a `ZCFMDatabase` object.

 An `ZCFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `ZCFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `ZCZCFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    ZCZCFMDatabase *db = [ZCZCFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    ZCZCFMDatabase *db     = [ZCZCFMDatabase databaseWithPath:dbPath];
 ...
 @return `ZCZCFMDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `ZCZCFMDatabase` object.

 An `ZCZCFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `ZCZCFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `ZCZCFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    ZCFMDatabase *db = [ZCFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    ZCFMDatabase *db     = [ZCFMDatabase databaseWithPath:dbPath];

 (For more information on temporary and in-memory databases, read the sqlite documentation on the subject: [http://www.sqlite.org/inmemorydb.html](http://www.sqlite.org/inmemorydb.html))

 @param inPath Path of database file

 @return `ZCFMDatabase` object if successful; `nil` if failure.
...
/** Execute select statement
 ...
 Executing queries returns an `<ZCFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.

 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[ZCFMResultSet next]>`) from one record to the other.
 ...
 @return A `<ZCFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.

 @see ZCFMResultSet
 @see [`ZCFMResultSet next`](<[ZCFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 */

- (ZCFMResultSet *)executeQuery:(NSString*)sql, ...;
...
- (ZCFMResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (ZCFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (ZCFMResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (ZCFMResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(ZCFMDatabase *adb) {
        ...
        int rowCount = 0;
        ZCFMResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [ZCFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using ZCFMDatabase::storeableDateFormat.
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using FMDB you will not need to interact directly with `ZCFMStatement`, but rather with `<ZCFMDatabase>` and `<ZCFMResultSet>` only.
...
@interface ZCFMStatement : NSObject {
