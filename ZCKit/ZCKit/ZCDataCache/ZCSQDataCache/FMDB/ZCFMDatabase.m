...
@interface ZCFMDatabase ()

- (ZCFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;

@end

@implementation ZCFMDatabase
...
#pragma mark ZCFMDatabase instantiation and deallocation

+ (instancetype)databaseWithPath:(NSString*)aPath {
    return ZCFMDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}
...
- (void)dealloc {
    [self close];
    ZCFMDBRelease(_openResultSets);
    ZCFMDBRelease(_cachedStatements);
    ZCFMDBRelease(_dateFormat);
    ZCFMDBRelease(_databasePath);
    ZCFMDBRelease(_openFunctions);
...
// if ([ZCFMDatabase FMDBVersion] >= 0x0230) { … }
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
    ZCFMDatabase *self = (__bridge ZCFMDatabase*)f;
...
// but for folks who don't bother noticing that the interface to ZCFMDatabase changed,
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = ZCFMDBReturnAutoreleased([_openResultSets copy]);
...
- (void)resultSetDidClose:(ZCFMResultSet *)resultSet {
...
- (ZCFMStatement*)cachedStatementForQuery:(NSString*)query {

    NSMutableSet* statements = [_cachedStatements objectForKey:query];

    return [[statements objectsPassingTest:^BOOL(ZCFMStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(ZCFMStatement*)statement forQuery:(NSString*)query {
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = ZCFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = ZCFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    ZCFMDBAutorelease(_dateFormat);
    _dateFormat = ZCFMDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    ZCFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The ZCFMDatabase %@ is currently in use.", self);

#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The ZCFMDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        NSLog(@"The ZCFMDatabase %@ is not open.", self);

    #ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The ZCFMDatabase %@ is not open.", self);
            abort();
        }
    #endif
...
- (NSError*)errorWithMessage:(NSString*)message {
    ...
    return [NSError errorWithDomain:@"ZCFMDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (ZCFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    ZCFMStatement *statement  = 0x00;
    ZCFMResultSet *rs         = 0x00;
    ...
    ZCFMDBRetain(statement); // to balance the release below

    if (!statement) {
        statement = [[ZCFMStatement alloc] init];
    ...
    rs = [ZCFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (ZCFMResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[ZCFMStatement alloc] init];

        [cachedStmt setStatement:pStmt];

        [self setCachedStatement:cachedStmt forQuery:sql];

        ZCFMDBRelease(cachedStmt);
    }
...
int ZCFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int ZCFMDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? ZCFMDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
void ZCFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void ZCFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
    ...
    id b = ZCFMDBReturnAutoreleased([block copy]);

    [_openFunctions addObject:b];

    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &ZCFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &ZCFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
